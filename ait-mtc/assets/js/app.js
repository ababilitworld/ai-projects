...
class RoadmapView {
  ...
  render() {
    ...
    this.root.innerHTML='<ol class="timeline">'+f.nodes.map((n,i)=>{
      ...
      return `<li class="${active?'current':n.confirmed?'done':'pending'} ${n.type==='speech'&&[1,4,5].includes(n.g)?'branch-lane':''} ${n.type==='speech'&&n.g===1&&n.qaIndex===0?'lane-start':''} ${n.type==='speech'&&n.g===5?'lane-end':''}"><button data-visit="${n.id}" ${available?'':'disabled'} ${active?'aria-current="step"':''}><span class="node-dot">${n.confirmed&&!active?'✓':String(i+1).padStart(2,'0')}</span><span><strong>${e(f.title(n))}</strong>${context?`<small>${e(context)}</small>`:''}</span>${active?'<span class="here">NOW</span>':''}</button></li>`;
...
class WorkspaceView {
  ...
  render() {
    ...
    if(n.type==='topic') {
      ...
      eyebrow=f.data[n.t].title; hint='Choose a subtopic. Each one has at least three paired questions and answers before Invitation and Response.';
      content='<div class="choices">'+f.data[n.t].subtopics.map((s,i)=>this.choice('subtopic',i,s.title,`${s.questionAnswers.length} Question & Answer rounds → Invitation → Response`,n.s===i)).join('')+'</div>';
      ...
      hint=n.g===0?'Start warmly. Choose a way to say hello, then explore a topic.':n.g===1?'Choose one paired question and answer. Both lines change together when you choose a pattern.':'Choose how to say it, add a mood, and practice the exchange aloud.';
