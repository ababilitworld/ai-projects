...
Package v18 sortable and filterable data tables
------------------------------------------------
- Fundamental Info and every scanner-output table now share one reusable ES6
  table component.
- Click a field heading to sort ascending; click it again to sort descending.
- Click additional headings to build a stable multi-column sort. Numbered badges
  show the active priority order.
- Each field has its own case-insensitive filter.
- Numeric/date filters accept >, >=, <, <=, =, != and inclusive ranges such as
  10..20. A vertical bar (|) matches alternative values.
- Clear Sort restores the scanner's original calculated order without changing
  its rank values. Clear Filters removes only the field filters.
- Sorting and field filters remain active when a scanner rerenders its tbody.
- Existing scanner search, responsive scrolling, themes and row actions remain
  compatible.


Package v19 active-watch-list scanner universe and menu hierarchy
-----------------------------------------------------------------
- Every scanner now calculates only the trading codes in the currently active
  watch list. Symbols from other lists, the mother list and history-only data
  are excluded from scanner rankings and relative comparisons.
- Technical, Smart Money, Relative Strength, Composite, Elite, Signal Priority,
  Historical, Advanced and Elite Regime scanners share the same active universe.
- Historical calendars, regime breadth and Elite performance benchmarks are
  also reconstructed only from active-watch-list codes.
- Scanner, historical, advanced, Elite and performance cache signatures include
  the active list and its codes, preventing results from a previous list from
  being reused after the selection changes.
- Scanner is now located at Workspace > Watch List > Scanner. It was removed
  from the Trading category, and its Back action returns to Watch List.
- Explorer and Portfolio symbol selectors retain their broader locally known
  symbol universe; this isolation applies specifically to scanner calculations.


Package v20 Data Center Report menu hierarchy
---------------------------------------------
- Moved the Charts, Report and Explorer workspaces from Trading > Report to
  Workspace > Data Center > Report.
- Removed the now-empty Report category from Trading; Trading now contains the
  Portfolio workspace only.
- Renamed the existing Data Center chart-gallery entry to Saved Charts so it is
  distinct from the multi-chart analysis workspace.
- Charts, Report and Explorer retain their original workspaces and now return to
  Data Center Report when their Back control is used.


Package v21 active-watch-list Data Center downloads and cache
--------------------------------------------------------------
- Watch List is now the first item shown in Workspace, followed by Data Center
  and Trading.
- Every Data Center Download level displays the active watch-list name, trading-
  code count, OHLC coverage, stored-row count, fundamentals coverage and cached
  date range.
- DSE Fundamentals, AmarStock Fundamentals, Instant OHLC, Incremental OHLC,
  Force 3M/6M/1Y, custom Range and Range CSV downloads are scoped to the active
  watch list.
- The PHP archive endpoint now applies an OOP ArchiveScope and returns or exports
  only requested active-list symbols. Its reusable full-market source cache is
  retained so overlapping watch lists do not trigger redundant upstream work.
- A versioned per-watch-list manifest records OHLC ranges, download modes,
  matched/missing symbols and fundamental-source coverage. Actual market rows
  remain normalized by trading code in IndexedDB, avoiding large duplication.
- Switching back to a watch list immediately restores its available OHLC and
  fundamentals from the shared local symbol cache and reports the restored
  counts.
- Incremental OHLC uses the oldest latest date across every active-list code.
  If any active code has no cached history, a selected 3M/6M/1Y operation safely
  initializes the full range instead of skipping that code.
- Backup schema v3 includes the watch-list download manifests; legacy backups
