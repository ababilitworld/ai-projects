...
(()=>{"use strict";
 ...
 const calculate=()=>{
  ...
  const sig=`${advanced.rows?.map(r=>`${r.code}:${Number(r.advancedScore||0).toFixed(2)}`).join('|')||''}|${primaryRows.map(r=>`${r.code}:${Number(r.primaryScore||0).toFixed(1)}:${r.rank||0}:${r.signalRank||0}`).join('|')}|${bridge()?.scannerHistorySignature?.()||bridge()?.scannerUniverseSignature?.()||''}`;
