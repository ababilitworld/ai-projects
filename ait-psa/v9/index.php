...
.ait-psa-terminal-modal-shell.open #aitPsaWatchlistModal:not([hidden]) .ait-psa-watchlist-layout{display:grid!important}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const modalActivityNames = {
    ...
    aitPsaDataReportMenuModal: 'Data Report',
...
(()=>{"use strict";
 ...
 const availableDates=()=>{const cutoff=window.__AIT_HISTORICAL_CUTOFF_DATE__||null;return [...new Set(Object.values(marketHistory()).flatMap(rows=>Array.isArray(rows)?rows.map(row=>String(row?.date||"")).filter(Boolean):[]))].filter(date=>!cutoff||date<=cutoff).sort()};
...
(()=>{"use strict";
 ...
 const dates=()=>{const cutoff=window.__AIT_HISTORICAL_CUTOFF_DATE__||null;return [...new Set(Object.values(history()).flatMap(rows=>Array.isArray(rows)?rows.map(r=>String(r?.date||"")).filter(Boolean):[]))].filter(date=>!cutoff||date<=cutoff).sort()};
...
(()=>{"use strict";
 ...
 const decisionLayer=r=>{
  const modelState=storedModelState(),shortTerm=shortTermSignal(r),midTerm=midTermSignal(r),longTerm="Not Validated",entryStateValue=entryState(r);
