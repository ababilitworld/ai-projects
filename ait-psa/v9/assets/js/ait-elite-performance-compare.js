...
(()=>{"use strict";
 ...
 const pct=(value,plus=false)=>!Number.isFinite(value)?'—':`${plus&&value>=0?'+':''}${value.toFixed(2)}%`;
 ...
 const rank=value=>{
  const rows=TYPES.flatMap(type=>DAYS.map(days=>{
   const stats=value.rows?.[key(type,days)];
   const hasData=!!stats&&stats.n>0&&stats.dates>0&&['raw','benchmark','excess','rawWin','excessWin'].every(field=>Number.isFinite(stats[field]));
   const eligible=hasData&&stats.dates>=20&&stats.n>=30;
   return {type,days,stats,id:key(type,days),name:label(type,days),hasData,eligible,
    excessPerSession:hasData?stats.excess/days:null,rawPerSession:hasData?stats.raw/days:null};
  }));
  // Round at the displayed precision so visually equal metrics share a rank.
  const score=row=>[Number(row.excessPerSession.toFixed(4)),Number(row.stats.excessWin.toFixed(2)),Number(row.rawPerSession.toFixed(4))];
  const compare=(a,b)=>{const x=score(a),y=score(b);for(let i=0;i<x.length;i++)if(x[i]!==y[i])return y[i]-x[i];return 0};
  rows.sort((a,b)=>Number(b.eligible)-Number(a.eligible)||Number(b.hasData)-Number(a.hasData)||(a.hasData?compare(a,b):0)||a.name.localeCompare(b.name));
  let previous=null,place=0;
  rows.forEach((row,index)=>{if(!row.eligible){row.rank=null;return}if(!previous||compare(previous,row)!==0)place=index+1;row.rank=place;previous=row});
  const eligible=rows.filter(row=>row.eligible),leaders=eligible.filter(row=>row.rank===1);
  let summary='No winner yet. Each scanner needs at least 20 signal dates and 30 evaluated samples to receive a rank.';
  if(eligible.length===1)summary=`Only ${eligible[0].name} meets the evidence minimum; at least two eligible scanners are needed to compare a winner.`;
  if(eligible.length>=2){
   summary=`${leaders.length>1?'Joint historical leaders':'Best observed historical performer'}: ${leaders.map(row=>row.name).join(' / ')}. Ranked among ${eligible.length} eligible scanners by excess return per session, then excess win rate, then raw return per session.`;
   if(leaders[0].excessPerSession<=0)summary+=' Even the highest-ranked result did not beat the benchmark on average.';
   else if(leaders[0].stats.raw<=0)summary+=' The leading result still lost value on average despite outperforming the benchmark.';
  }
  return {rows,summary};
 };
 const perSession=value=>value==null?'—':`${value>=0?'+':''}${value.toFixed(4)}%`;
 const comparisonRow=row=>{const {type,days,stats}=row;return `<tr data-monitor="${key(type,days)}"><td><strong>${row.rank==null?'—':`#${row.rank}`}</strong></td><td><strong>${label(type,days)}</strong></td><td>${row.eligible?'Eligible':row.hasData?'Limited evidence':'No data'}</td><td><strong>${perSession(row.excessPerSession)}</strong></td><td>${stats?.dates??0}</td><td>${stats?.n??0}</td><td>${pct(stats?.raw)}</td><td>${pct(stats?.benchmark)}</td><td>${pct(stats?.excess,true)}</td><td>${pct(stats?.rawWin)}</td><td>${pct(stats?.excessWin)}</td></tr>`};
 const render=value=>{
  ...
  const ranking=rank(value),winner=byId('aitEliteCompareWinner');
  if(winner)winner.textContent=ranking.summary;
  body.innerHTML=ranking.rows.map(comparisonRow).join('');
 ...
 const init=()=>{
  ...
  const compare=document.createElement('button');compare.type='button';compare.className='ait-psa-terminal-command';compare.dataset.aitPsaOpen='aitEliteCompareModal';compare.innerHTML='<span>⇄</span><b>Compare 2D, 3D, 6D and 9D</b><small>Calculate all eight scanner performances and rank them from best to worst.</small>';menu.replaceChildren(compare);
  compare.addEventListener('click',()=>{selected='all';setTimeout(()=>run(true).catch(()=>{}),0)});
