// Comparable chronological replay for the Elite and Elite Regime 3D, 6D and 9D scanners.
(()=>{"use strict";
 const DAYS=[3,6,9],TYPES=['elite','regime'];
 ...
 const storageKey='ait-psa-elite-horizon-comparison-v1';
 let memory=null,running=null,selected='all';
 const cached=sig=>{if(memory?.signature===sig)return memory;try{const found=JSON.parse(localStorage.getItem(storageKey)||'null');if(found?.signature===sig&&found?.version===1)return memory=found}catch(_){}return null};
 ...
 const replay=async()=>{
  ...
  const value={version:1,signature:sig,builtAt:new Date().toISOString(),period:{from:dates[start]||null,to:dates[Math.max(start,end-1)]||null,dates:Math.max(0,end-start)},rows:Object.fromEntries([...totals].map(([id,bucket])=>[id,pack(bucket)])),details:Object.fromEntries([...details].map(([id,groups])=>[id,Object.fromEntries([...groups].map(([name,bucket])=>[name,pack(bucket)]))]))};
  return save(value);
 };
 ...
 const init=()=>{
  ...
  for(const type of TYPES)for(const days of [3,6]){const button=document.createElement('button');button.type='button';button.className='ait-psa-terminal-command';button.dataset.aitPsaOpen='aitEliteCompareModal';button.dataset.monitor=key(type,days);button.innerHTML=`<span>${type==='regime'?'◈':'✹'}</span><b>${label(type,days)}</b><small>${days}D historical outcomes and the 3D / 6D / 9D comparison.</small>`;const anchor=menu.querySelector(`[data-ait-psa-open="${type==='regime'?'aitEliteRegimePerformanceModal':'aitElitePerformanceModal'}"]`);anchor?.before(button)}
  const compare=document.createElement('button');compare.type='button';compare.className='ait-psa-terminal-command';compare.dataset.aitPsaOpen='aitEliteCompareModal';compare.dataset.monitor='all';compare.innerHTML='<span>⇄</span><b>Compare 3D, 6D and 9D</b><small>Six scanners in one table, with signal and regime breakdowns.</small>';menu.appendChild(compare);
